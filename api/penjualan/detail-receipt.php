<?php
/**
 * File: api/penjualan/detail-receipt.php
 * Deskripsi: Mengambil detail 1 Penerimaan Penjualan berdasarkan ID atau Nomor
 */
...
if (empty($id) && empty($number)) {
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Parameter id atau number (Nomor Penerimaan Penjualan) wajib diisi.'
    ], JSON_PRETTY_PRINT);
    exit;
}

// Panggil fungsi dari AccurateAPI.php
$result = $api->getSalesReceiptDetail($id, $number);
...
if ($result['success']) {
    ...
    echo json_encode([
        'status' => 'error',
        'message' => $result['error'] ? $result['error'] : 'Data Penerimaan Penjualan tidak ditemukan atau terjadi kesalahan'
    ], JSON_PRETTY_PRINT);
}
?>
